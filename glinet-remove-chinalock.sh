...
invoke_convert_countrycode() {
    ...
    # Add FF to beginning of the country code
    COUNTRY_CODE_HEX="FF${COUNTRY_CODE_HEX}"
    echo $COUNTRY_CODE_HEX
}

invoke_find_partition() {
    log "INFO" "Searching for the factory partition ..."
    FACTORY_PARTITION=$(blkid -t PARTLABEL="factory" -o device)
    if [ -z "$FACTORY_PARTITION" ]; then
        log "ERROR" "Could not find the factory partition"
        log "ERROR" "Please report this issue to the GitHub repository."
        exit 1
    else
        log "SUCCESS" "Found the factory partition at $FACTORY_PARTITION"
    fi
}

invoke_backup() {
    log "INFO" "Backing up the factory partition ..."
...
invoke_find_countrycode()
{
    ...
    log "INFO" "Searching for the country code $COUNTRY_CODE ($COUNTRY_CODE_HEX) in the factory partition $DEVICE ..."
    BINARY_OFFSET=$(hexdump -ve '1/1 "%.2X"' $DEVICE | tr -d '\n' | sed -n "s/\(.*\)$COUNTRY_CODE_HEX\(.*\)/\1/p" | wc -c)
    if [ -z $BINARY_OFFSET ]; then
        log "ERROR" "Could not find the country code ($COUNTRY_CODE_HEX) in the factory partition"
        log "ERROR" "Please report this issue to the GitHub repository."
        exit 1
    else
        ...
        log "SUCCESS" "Found the country code ($COUNTRY_CODE_HEX) at offset $BINARY_OFFSET"
    fi
}
...
invoke_ask() {
    ...
    echo -e "${RED}It could DESTROY your device and VOID your warranty!${INFO}"
...
invoke_update() {
     SCRIPT_VERSION_NEW=$(curl -s "https://raw.githubusercontent.com/Admonstrator/glinet-remove-chinalock/main/glinet-remove-chinalock.sh" | grep -o 'SCRIPT_VERSION="[0-9]\{4\}\.[0-9]\{2\}\.[0-9]\{2\}\.[0-9]\{2\}"' | cut -d '"' -f 2 || echo "Failed to retrieve script version")
    if [ "$SCRIPT_VERSION_NEW" != "$SCRIPT_VERSION" ]; then
        ...
        wget -qO /tmp/update-tailscale.sh "https://raw.githubusercontent.com/Admonstrator/glinet-remove-chinalock/main/glinet-remove-chinalock.sh"
...
invoke_find_partition
invoke_backup
invoke_find_countrycode $FACTORY_PARTITION $COUNTRY_CODE $COUNTRY_CODE_HEX
invoke_remove_china_lock
invoke_done
exit 0